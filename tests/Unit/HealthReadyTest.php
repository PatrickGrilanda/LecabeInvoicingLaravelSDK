...
final class HealthReadyTest extends TestCase
{
    ...
    public function testReadyViaInvoicingClientOmitsAuthHeaders(): void
    {
        $container = [];
        $history = Middleware::history($container);
        $payload = json_encode([
        ...
        $client = new InvoicingClient(new Config(apiKey: 'should-not-leak-on-ready'), $http);
        ...
        self::assertCount(1, $container);
        $req = $container[0]['request'];
        self::assertStringEndsWith('/ready', $req->getUri()->getPath());
        self::assertSame('', $req->getHeaderLine('X-API-Key'));
        self::assertSame('', $req->getHeaderLine('Authorization'));
    }
